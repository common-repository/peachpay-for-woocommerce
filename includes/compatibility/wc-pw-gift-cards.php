 ...
 */
function peachpay_pwgc_record( $record, $cart, $currency_code ) {
	if ( isset( $cart->pwgc_calculated_total ) ) {
		...
		foreach ( $session_data['gift_cards']  as $card_number => $amount ) {
			...
			$record[ $card_number ] = peachpay_convert_currency_rest_api( floatval( $applied_amount ), $currency_code );
		}
	}
	return $record;
}
add_filter( 'peachpay_cart_applied_gift_cards_record', 'peachpay_pwgc_record', 10, 3 );
...
/**
 * Applies any present gift cards for a cart calculation.
 *
 * @param array $data The data being used for a cart calculation.
 */
function peachpay_pwgc_gift_card_apply( $data ) {
	if ( isset( $data[ PP_GIFT_CARDS ] ) && ! empty( $data[ PP_GIFT_CARDS ] ) ) {
		global $pw_gift_cards_redeeming;
		foreach ( $data[ PP_GIFT_CARDS ] as $gift_card ) {
			$pw_gift_cards_redeeming->add_gift_card_to_session( $gift_card );
		}
	}
}
add_action( 'peachpay_calculate_carts_init', 'peachpay_pwgc_gift_card_apply', 10, 1 );


/**
 * Cleans up after calculating carts.
 */
function peachpay_pwgc_calculate_carts_cleanup() {
	WC()->session->set( PWGC_SESSION_KEY, null );
}
add_action( 'peachpay_calculate_carts_cleanup', 'peachpay_pwgc_calculate_carts_cleanup' );
