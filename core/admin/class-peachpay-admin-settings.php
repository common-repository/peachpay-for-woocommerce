...
/**
 * PeachPay Admin Settings page manager.
 *
 * @deprecated Use Peachpay_Admin instead.
 */
class PeachPay_Admin_Settings {

	/**
	 * Controller object for the analytics pages.
	 *
	 * @var PeachPay_Analytics_Controller
	 */
	private static $analytics_page;

	/**
	 * Magic constructor function is called on instantiation.
	 *
	 * @param PeachPay_Database $db The database object to be passed to children.
	 */
	public function __construct( $db = null ) {
		self::$analytics_page = new PeachPay_Analytics_Controller( $db );
		add_action( 'admin_menu', array( $this, 'peachpay_options_page' ) );
	}

	/**
	 * Registers peachpay sidebar link.
	 */
	public function peachpay_options_page() {
		add_menu_page(
		...
		add_submenu_page(
			...
			'manage_options',
			'peachpay_analytics',
			array( self::$analytics_page, 'peachpay_analytics_page_init' )
		);
	}
...
function peachpay_generate_nav_bar() {
	...
	<nav class="nav-tab-wrapper woo-nav-tab-wrapper">
		...
		> <?php esc_html_e( 'Payment', 'peachpay-for-woocommerce' ); ?>
		</a>
		<a
		...
		> <?php esc_html_e( 'Product recommendations', 'peachpay-for-woocommerce' ); ?>
		</a>
		<a
		...
		><?php esc_html_e( 'Advanced', 'peachpay-for-woocommerce' ); ?>
		</a>
		<!--
			We're going to hold off on the landing page until after the Jan. 15 WooCommerce promotion, so we can focus
			on making the plugin as stable as possible. Also, rough plan is to rebrand landing page to "Product Links"
		-->
		<!--
		<a
			class="nav-tab <?php echo 'product_links' === $tab ? 'nav-tab-active' : ''; ?>"
			href="<?php echo esc_url( add_query_arg( 'tab', 'product_links' ) ); ?>"
		><?php esc_html_e( 'Product Links', 'peachpay-for-woocommerce' ); ?>
		</a>
		--->
	</nav>
	<?php
...
define(
	...
	array(
		...
		'square-register-apple-pay-domain',
	)
);
