...
abstract class PeachPay_Stripe_Payment_Gateway extends PeachPay_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		try {
			...
			if ( 0.0 === floatval( $order->get_total() ) ) {
				return $this->process_zero_total_payment( $order, $payment_method_id, $save_to_account, $token_id );
			}
	...
	public function process_subscription_renewal( $parent_order, $renewal_order, $renewal_total ) {
		try {
			...
			$payment_intent_params = array(
				...
				'payment_method'              => $payment_method_id,
				'payment_method_types'        => array( $this->stripe_payment_method_type, 'link' ),
	 ...
	 * @param string   $save_to_account If the payment should be saved to the account.
	 * @param string   $token_id The token of which to save.
	 */
	public function process_zero_total_payment( $order, $payment_method_id, $save_to_account, $token_id ) {
		$payment_method_details = null;
		...
		if ( $this->supports( 'tokenization' ) && get_current_user_id() !== 0 ) {
			if ( ( 'true' === $save_to_account || '1' === $save_to_account ) && ( 'new' === $token_id || null === $token_id ) ) {
				$this->create_payment_token( $order, get_current_user_id() );
			}
		}

		self::set_payment_method_title( $order );
		PeachPay_Stripe::set_customer( get_current_user_id(), $this->get_stripe_customer( get_current_user_id() ) );
