...
function peachpay_handle_stripe_onhold_status( $order, $request ) {
	...
	if ( get_post_meta( $order->get_id(), 'payment_method_variation', true ) === 'us_bank_account' ) {
		return 'Payment is processing in Stripe. Order is on-hold until processing completes.';
	} else {
		return 'Payment authorization successful in Stripe. Charge: ' . $stripe_meta_result[1] . '. Change the order status to completed or manually specify an amount in the charge section to capture the payment. The authorized payment will expire after 7 days.';
	}
}
