...
function peachpay_get_paypal_merchant_id() {
	...
	$response          = wp_remote_get( peachpay_api_url( peachpay_is_test_mode() ? 'test' : 'prod' ) . 'api/v1/paypal/merchantAndClient?merchantHostname=' . $merchant_hostname );
...
function peachpay_paypal_signup_url() {
	$response = wp_remote_get( peachpay_api_url( peachpay_is_test_mode( 'prod' ) ) . 'api/v1/paypal/signup?merchant_url=' . get_home_url() . '&wp_admin_url=' . get_site_url() );
