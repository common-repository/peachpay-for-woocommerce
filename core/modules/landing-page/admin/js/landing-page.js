window.addEventListener('DOMContentLoaded' , ()=> {
	...
	if (params.get('page') !== 'peachpay' && params.get('tab') !== 'landing_page') {
		return;
	}
	getActiveProducts();
})
...
document.querySelector('#peachpay-register-landing')?.addEventListener('click', async (event)=>{
	...
	const response = await fetch(`${pp_landing_data.URL}/api/register`, {
		method : "POST",
		headers:{
	...
	document.querySelector('#landing_page_key').value = data.merchantID;

	document.querySelector('#hidden_submit').click();
})

async function getActiveProducts() {
	const merchantID = document.querySelector('#landingKey')?.value;
	if (!merchantID) {
		return;
	}
	const response = await fetch(`${pp_landing_data.URL}/api/getProducts`, {
		method:"POST",
		headers: {
			"content-type" : "application/json",
		},
...
function buildProduct( product ) {
	...
	loadDiv.classList.add("peachpay-landing-product-parent");
	const productDiv = document.createElement('div');
	productDiv.classList.add('peachpay-landing-product');
	...
	removeContainer.classList.add('pp-landing-remove-container');
	const removeButton = document.createElement('div');
	removeButton.classList.add('pp-landing-remove');
...
async function removeProductFromStore( productID, parent, product ) {
	...
	const merchantID = document.querySelector('#landingKey')?.value;
	const response = await fetch(`${pp_landing_data.URL}/api/deleteProduct`, {
		method:"POST",
		headers: {
			"content-type" : "application/json",
		},
