...
class PeachPay_Analytics_Database {
	...
	public static function query_analytics( $query ) {
		...
		if ( $is_interval && ! self::check_table_existence( $analytics_interval_table ) ) {
			if ( $is_interval ) {
		...
		if ( array_key_exists( 'currency', (array) $query ) ) {
			...
			if ( $currency_convert ) {
				...
				if ( self::check_table_existence( $analytics_table ) ) {
				...
				if ( $currency_options && ! empty( $currency_options ) ) {
					foreach ( $currency_options as $currency_option ) {
						...
						if ( property_exists( $data, 'error' ) && $data->error ) {
							peachpay_notify_error( new Exception( $data->error ) );
							continue;
						}
		...
		try {
			...
			if ( ! $active_currency_error && self::check_table_existence( $analytics_table ) ) {
		...
		if ( ! $is_interval ) {
			...
			for ( $analytic_index = 0; $analytic_index < $analytic_meta_data_count; $analytic_index++ ) {
				...
				$currency_rate = isset( $update_active_currencies[ $analytic_meta_data[ $analytic_index ]->currency ] ) &&
                    ...
                    !empty($update_active_currencies[ $analytic_meta_data[ $analytic_index ]->currency ]['rate']) ?
					$update_active_currencies[ $analytic_meta_data[ $analytic_index ]->currency ]['rate'] : 1;
				...
				$analytic_count_data['datasets'][0]['data'][ $grab_currency_index ]            = $currency_convert ?
					$analytic_meta_data[ $analytic_index ]->value / $currency_rate :
		...
		for ( $grab_data_by_key = 0; $grab_data_by_key < $analytic_meta_data_count; $grab_data_by_key++ ) {
			...
			/**
			 * Initialize entire data array as below the array is inserted in reverse order which would cause issues
			 * attempting to convert to JS array on the frontend.
			 */
			for ( $dates_index = 0; $dates_index < $dates_length; $dates_index++ ) {
				$analytics_interval[ $grab_currency_index ]['data'][ $dates_index ] = 0;
			}

			$interval_data_index = count( $interval_data ) - 1;

			/**
			 * Loop through all date indices while searching for the correct position for each interval value from the
			 *  data base response. This starts from the final date and slowly looks towards the first date while
			 *  contiously placing data into position as necessary.
			 */
			for ( $dates_index = $dates_length - 1; $dates_index >= 0; $dates_index-- ) {
