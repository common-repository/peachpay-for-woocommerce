...
function peachpay_analytics_update_cart() {
	...
	try {
		...
		if ( ! $cart_id || $returning ) {
			...
			if ( ! $cart_id ) {
				/**
				 * Attempt to use MAX(cart_id) instead in case the LAST_INSERT_ID() returns 0 (see https://stackoverflow.com/a/13882839).
				 * There's a small chance that this could result in an incorrect cart_id result, but will be much more
				 * accurate than all cart ids return 0.
				 */
				$cart_id = $wpdb->get_var( "SELECT MAX(cart_id) FROM {$cart_meta_table};" );
			}
...
function wp_ajax_query_analytics() {
	// Ignoring POST warnings as query is sanitized and then solely used in prepared statements in the query_analytics
	//  method which never saves or directly uses contents.
	//phpcs:ignore
	$post = $_POST;
...
/**
 * Drops all analytics tables.
 */
function peachpay_purge_cart_analytics() {
	if ( ! empty( $_POST ) && isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'peachpay-purge-cart-analytics' ) ) {
		PeachPay_Analytics_Database::drop_tables();

		wp_safe_redirect( isset( $_POST['_wp_http_referer'] ) ? wp_unslash( $_POST['_wp_http_referer'] ) : '/' );
		exit;
	}
}
